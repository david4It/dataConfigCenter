...
public interface DashboardPortalMapper {
    ...
    @Delete({"delete from daav_dashboard_portal where id = #{id}"})
    int deleteById(@Param("id") Long id);


    @Select({"select * from daav_dashboard_portal where id = #{id}"})
    DashboardPortal getById(@Param("id") Long id);


    @Update({
            "update daav_dashboard_portal",
    ...
    @Select({"select id from daav_dashboard_portal where project_id = #{projectId} and name = #{name}"})
    Long getByNameWithProjectId(@Param("name") String name, @Param("projectId") Long projectId);

    @Select({"select * from daav_dashboard_portal where project_id = #{projectId}"})
