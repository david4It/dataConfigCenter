...
public interface RelRoleViewMapper {
    ...
    @Delete({
            "delete from daav_rel_role_view where role_id = #{roleId} and view_id = #{viewId}"
    })
    int delete(@Param("roleId") Long roleId, @Param("viewId") Long viewId);

    @Update({
            "update daav_rel_role_view",
    ...
    @Delete({
            "delete from daav_rel_role_view where  view_id = #{viewId}"
    })
    int deleteByViewId(Long viewId);


    @Delete({
            "delete from daav_rel_role_view where  role_id = #{roleId}"
    })
    int deleteByRoleId(Long roleId);

    @Select({
            "select rrv.* from daav_rel_role_view rrv",
            "       left join `daav_view` v on v.id = rrv.view_id",
            "       left join daav_rel_role_user rru on rru.role_id = rrv.role_id",
    ...
    @Select({
           "select * from daav_rel_role_view where  view_id = #{viewId}"
    })
    List<RelRoleView> getByView(Long viewId);

    @Delete({
            "delete from daav_rel_role_view where view_id in (select id from daav_view where project_id = #{projectId})"
    })
