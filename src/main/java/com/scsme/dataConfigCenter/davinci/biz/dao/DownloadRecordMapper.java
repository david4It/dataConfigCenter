...
public interface DownloadRecordMapper {
    ...
    @Delete({
            "delete from daav_download_record where id = #{id,jdbcType=BIGINT}"
    })
    int deleteById(Long id);

    @Select({
            "select * from daav_download_record where id = #{id,jdbcType=BIGINT}"
    })
    DownloadRecord getById(Long id);


    @Delete({
            "delete from daav_download_record where create_time < DATE_FORMAT((NOW() - INTERVAL 1 MONTH),'%Y%m%d')"
    })
    int deleteBeforAMonthRecord();


    @Select({
            "select * from daav_download_record where user_id = #{userId} and create_time > DATE_FORMAT((NOW() - INTERVAL 7 DAY),'%Y%m%d')  order by create_time desc"
    })
    List<DownloadRecord> getDownloadRecordsByUser(Long userId);

    @Update({
            "update daav_download_record",
