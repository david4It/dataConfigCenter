...
layui.use(['element', 'form', 'layedit', 'laydate', 'colorpicker','table','ace.min'], function(){
   ...
   */
	...
	document.ondrop=function(ev){
		...
		appendDragValue(data,ev.target);
		//monitor charts change
		moniteChartsChange();
		//end monitor
	}
...
/**
 * generate dragged tag
 * @param data dragged data
 * @param targetItem html element
 */
function appendDragValue(data,targetItem) {
 ...
 * generate dragged tag
 * @param data dragged data
 * @param targetItem html dom
 * @param cat category,value or filters
 */
function initialSelectedDragValue(data,targetItem,cat) {
	//console.log("item: ", item);
	//console.log("item.lay-filter: ",item.getAttribute("lay-filter"));
	let newLi = document.createElement("li");
	newLi.innerText=data;
	...
	if(cat ==="value" && targetId ==="dragedValue"){
		...
		let doc = document.getElementById("dragedDimesion");
		doc.appendChild(newLi);
	}else if(cat ==="filter" && targetId ==="dragedFilter"){
		//fiter
		newLi.setAttribute("id","filter_"+data);
		let doc = document.getElementById("dragedFilter");
		doc.appendChild(newLi);
...
function saveWidget() {
	...
	$.ajax({
		...
		type: "PUT",
		data: JSON.stringify({"id":id,"projectId":projectId,"viewId":viewId,"name":name,"type":1,"uid":$.cookie("uid"),"config":JSON.stringify(config),"description":description,"publish":true}),
		dataType: "json",
		contentType: "application/json;charset=utf-8",
		xhrFields: {
			withCredentials: true //允许跨域带Cookie
