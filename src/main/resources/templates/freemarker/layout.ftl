...
<div style="width: ${vo.getWidth()?c}%; height: ${(vo.getHeight() * 80)?c}px; position: absolute; top: ${(vo.getY() * 100)?c}px; left: ${(vo.getX())?c}%; padding: 15px;">
    <dv-border-box-8 v-show="false">
        ...
        <#if vo.getType() == 'table'>
            <dv-scroll-board :key="tableKey" id="${'component_' + vo.getLocationIndex()}" :config="tableConfig" <#if vo.getLinkUrl()??>@click="rowClick"</#if> style="width:100%;height: ${(vo.getHeight() * 100 - 80)?c}px; padding: 5px;" />
        <#else>
